...
const Analyze = () => {
    ...
    const [text, setText] = useState("all post types")
    ...
    const callLangflowAPI = async (inputBody) => {
        ...
        try {
            ...
            alert("Error calling the Langflow API:", error)
            setLoading(false)
        }
    }
    ...
    useEffect(() => {
        setLoading(true)
        callLangflowAPI({ "text": text })
    }, [text])

    return (
        loading ? <Loader /> : (
            <div className='py-12 md:py-16 px-4 md:px-16 flex flex-col gap-16'>
                <Selector text={text} setText={setText} />
